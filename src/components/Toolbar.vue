<script setup>
	...
	onUpdated (() => {
        ...
        switch(props.path) {
            case '/card':
                isCardPage.value = true;
                break;

            case '/collection':
                isCollectionPage.value = true;
                isCardPage.value = false;
                break;
...
<template>
    <div id="toolbar" :class="{active: showMenu}">
        <div class="buttons">
            ...
            <button v-else class="main" :class="{hidden: hideAction}" @click="handleNavigate('/deck')">
